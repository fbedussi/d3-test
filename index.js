...
function draw(data) {
  ...
  svg.append('g')
    ...
    .append('g')
    .attr('class', (d, index) => {
      const baseClass = `points ${index < 2 ? 'first ' : ''}`
      return d.eye === 'R' ? `${baseClass} purple-shadow` : `${baseClass} green-shadow`
    })
    .append("circle")
      .attr("cx", d => x(d.x))
      .attr("cy", d => y(d.y))
      .attr("r", 7)
      .style("stroke", d => d.eye === 'R' ? '#a34eff' : '#2fa59e')
      .style("fill", '#fff')
  ...
  svg.selectAll(".points")
    ...
    .attr("cy", d => y(d.y))
    .attr("r", 5)
    .style("fill", d => d.eye === 'R' ? '#a34eff' : '#2fa59e')
